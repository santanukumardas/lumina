    ...
    return (
        <div className="max-w-7xl mx-auto p-4 animate-slide-up select-none">
             <div className="text-center mb-8">
                <h2 className="text-3xl font-bold text-white mb-2 font-serif tracking-tight">ND Filter Lab</h2>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                {SCENES.map(scene => (
                    <button
                        ...
                        className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${activeSceneId === scene.id ? 'bg-zinc-800 border-blue-500/50 text-white shadow-lg shadow-blue-900/20' : 'bg-zinc-900/60 border-zinc-800 text-zinc-400 hover:bg-zinc-800'}`}
                    >
                        <div className={`p-2 rounded-lg ${activeSceneId === scene.id ? 'bg-blue-500/20 text-blue-400' : 'bg-zinc-950 text-zinc-500'}`}>
                            <scene.icon size={18} />
                        </div>
                        <div className="text-left">
                            <div className="text-sm font-bold font-serif tracking-tight">{scene.label}</div>
                            <div className="text-[10px] opacity-60 uppercase tracking-wider font-bold">Preset</div>
                        </div>
                    </button>
                ))}
            </div>
            ...
            <div className="flex flex-col lg:flex-row gap-8 mb-8">
                 ...
                 <div className="flex-1 space-y-4">
                     ...
                     >
                         ...
                         <div className="absolute inset-0">
                            ...
                            />
                            <div className="absolute top-4 right-4 bg-black/60 backdrop-blur-md px-3 py-1 rounded-lg text-xs font-bold text-white border border-white/10 font-mono">
                         ...
                         >
                            ...
                            />
                             {/* Alignment fix via background image */}
                             <div 
                                className="absolute inset-0 w-full h-full bg-cover bg-center"
                                style={{ backgroundImage: `url(${activeScene.img})` }}
                             />
                             
                             <div className="absolute top-4 left-4 bg-black/60 backdrop-blur-md px-3 py-1 rounded-lg text-xs font-bold text-white border border-white/10 font-mono">
                         ...
                         <div className="absolute bottom-0 w-full p-4 bg-gradient-to-t from-black/80 to-transparent flex justify-between text-xs text-zinc-300 pointer-events-none font-medium">
                             <span>Frozen (Fast Shutter)</span>
                             <span>Motion Blur (Slow Shutter)</span>
                         </div>
                     </div>
                     
                     <div className="flex gap-2 items-start bg-blue-900/10 border border-blue-500/20 p-4 rounded-xl backdrop-blur-sm">
                        <Info className="text-blue-400 shrink-0 mt-0.5" size={16} />
                        <p className="text-sm text-blue-200/80 leading-relaxed">
                            <strong>Why use ND?</strong> In bright light, you can't use a slow shutter speed (like <span className="font-mono">{formatSpeed(finalSpeed)}</span>) without overexposing. The ND filter acts like "sunglasses" for your lens.
                        </p>
                     </div>
                 </div>
                 ...
                 <div className="w-full lg:w-96 flex flex-col gap-6">
                      ...
                      {/* Base Settings */}
                      <div className="bg-zinc-900/60 backdrop-blur-md p-6 rounded-2xl border border-zinc-800">
                          ...
                          <select 
                            className="w-full bg-zinc-950 border border-zinc-700 text-white rounded-lg p-3 outline-none focus:border-blue-500 transition-colors font-mono text-sm"
                          ...
                          <div className="grid grid-cols-2 gap-3">
                              {[0, 3, 6, 10].map(s => (
                                  <button
                                    key={s}
                                    onClick={() => setStops(s)}
                                    className={`relative p-4 rounded-xl text-sm font-bold transition-all border flex flex-col items-center gap-2 ${stops === s ? 'bg-zinc-800 border-emerald-500 text-white ring-1 ring-emerald-500/50' : 'bg-zinc-950/50 border-zinc-800 text-zinc-400 hover:border-zinc-600'}`}
                                  >
                                      ...
                                      ></div>
                                      <span className="font-serif tracking-tight">{s === 0 ? 'No Filter' : `${s} Stops`}</span>
                      ...
                      {/* Calculation Result */}
                      <div className="bg-gradient-to-br from-zinc-800 to-zinc-900 p-6 rounded-2xl border border-zinc-700 mt-auto shadow-xl relative overflow-hidden">
                          ...
                          <div className="flex items-baseline gap-2 mb-2">
                                <span className="text-5xl font-bold text-white tracking-tighter font-mono">{formatSpeed(finalSpeed)}</span>
                          </div>
                          
                          <div className="w-full h-px bg-zinc-700 my-4"></div>
                          
                          <div className="flex justify-between items-center text-xs">
                              <span className="text-zinc-500 font-bold uppercase tracking-wider">Light Reduction</span>
                              <span className="text-emerald-400 font-mono">{stops} EV ({Math.pow(2, stops)}x darker)</span>
                          </div>
                          <div className="flex justify-between items-center text-xs mt-2">
                              <span className="text-zinc-500 font-bold uppercase tracking-wider">Time Multiplier</span>
