    ...
    return (
        <div className="max-w-6xl mx-auto p-4 animate-slide-up">
            <div className="text-center mb-8">
                <h2 className="text-3xl font-bold text-white mb-2 font-serif tracking-tight">Diffraction Limit Calculator</h2>
                <p className="text-zinc-400">Physics simulation of optical sharpness limits based on your sensor.</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                {/* Visualizer */}
                <div className="bg-zinc-950 border border-zinc-800 rounded-2xl p-8 relative flex flex-col items-center justify-center min-h-[400px] shadow-2xl">
                     <h3 className="absolute top-6 left-6 text-xs font-bold text-zinc-500 uppercase tracking-widest">Microscopic View</h3>
                     
                     {/* The Pixel Grid */}
                     <div className="relative w-64 h-64 grid grid-cols-4 grid-rows-4 border border-zinc-700 shadow-[0_0_20px_rgba(0,0,0,0.5)]">
                         {Array.from({length: 16}).map((_, i) => (
                             <div key={i} className="border border-zinc-800 bg-zinc-900/50 flex items-center justify-center backdrop-blur-sm">
                     ...
                     <div className="mt-8 text-center space-y-2">
                         <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold border backdrop-blur-md ${isSevere ? 'bg-red-900/20 border-red-500/50 text-red-400' : isDiffracted ? 'bg-amber-900/20 border-amber-500/50 text-amber-400' : 'bg-green-900/20 border-green-500/50 text-green-400'}`}>
                         ...
                         <p className="text-xs text-zinc-500 max-w-sm mx-auto font-mono">
                             Airy Disk: {airyDiskDiameter.toFixed(1)}µm vs Pixel: {pixelPitchMicrons.toFixed(1)}µm
                         </p>
                     </div>
                </div>
                ...
                <div className="flex flex-col gap-6">
                    {/* Settings Box */}
                    <div className="bg-zinc-900/60 backdrop-blur-md p-6 rounded-2xl border border-white/5 space-y-6">
                         <div>
                    ...
                    {/* Results Box */}
                    <div className="bg-gradient-to-br from-zinc-900 to-zinc-950 p-6 rounded-2xl border border-zinc-700 shadow-xl flex-1 flex flex-col justify-center relative overflow-hidden">
                         ...
                         <div className="flex items-center gap-3 mb-6 relative z-10">
                             ...
                             <div>
                                 <h4 className="text-sm font-bold text-white uppercase tracking-wide">Pixel Pitch</h4>
                                 <p className="text-xs text-zinc-400">Physical size of one pixel</p>
                             </div>
                             <div className="ml-auto text-xl font-mono text-blue-400 font-bold">{pixelPitchMicrons.toFixed(2)}µm</div>
                         </div>
                         ...
                         <div className="flex items-center gap-3 relative z-10">
                             ...
                             <div>
                                 <h4 className="text-sm font-bold text-white uppercase tracking-wide">DLA Limit</h4>
                         ...
                         <div className="mt-6 p-4 bg-black/40 rounded-lg text-xs text-zinc-400 leading-relaxed border border-white/5 relative z-10">
                             <strong>Pro Tip:</strong> For this {megapixels}MP camera, avoid stopping down past <strong className="text-white font-mono">f/{Math.floor(dla * 2) / 2}</strong>. Beyond this, the physics of light diffraction will make the image softer than the sensor's resolution.
                         </div>
                    </div>
                </div>
            </div>
